...
impl AocDay for Day {
    fn run_part1(&mut self, input: &[String]) -> Result<AoCResult> {
        ...
        let x_offsets: Vec<_> = (0..galaxy.width())
            .filter(|&x| galaxy.get_col(x).into_iter().all(|&g| !g))
            .collect();
        let y_offsets: Vec<_> = (0..galaxy.height())
            .filter(|&y| galaxy.get_row(y).into_iter().all(|&g| !g))
            .collect();
