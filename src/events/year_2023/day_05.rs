...
impl AocDay for Day {
    fn run_part1(&mut self, input: &Vec<String>) -> Result<AoCResult> {
        let mut seeds: Vec<usize> = input[0]
            ...
            .filter_map(|item| item.trim().parse().ok()).collect();
...
impl Map {
    fn new(ranges: &[String]) -> Self {
        ...
        for range in ranges.iter() {
            let parts: Vec<usize> = range.split_ascii_whitespace().filter_map(|item| item.trim().parse().ok()).collect();
    ...
    pub fn get_index(&self, nr: usize) -> usize {
        match self.source_ranges.iter().enumerate().filter(|&(_,  r)| r.contains(&nr)).next() {
            Some((index, range)) => {
